...
export class TimelineView extends BasesView implements HoverParent {
	...
	constructor(controller: QueryController, parentEl: HTMLElement) {
		...
		this.scrollerEl = this.rootEl.createDiv("ptl-scroller");
	...
	private render(): void {
		...
		for (const g of groups) {
			if (showGroupLabels) {
				...
				gh.createSpan({ cls: "ptl-group-label", text: g.label ?? "—" });
			}
			for (const item of g.items) this.renderRow(body, item);
		}
	...
	private renderHeader(canvas: HTMLElement, minDay: number, maxDay: number, today: number, tick: "day" | "week" | "month"): void {
		...
		// Top row: month labels, or year labels at year zoom.
		let d = minDay;
		while (d <= maxDay) {
			const date = dateOf(d);
		...
		for (let day = minDay; day <= maxDay; day++) {
			...
			if (tick === "day") {
				show = true;
				text = String(dateOf(day).getUTCDate());
			} else if (tick === "week" && isMonday(day)) {
				show = true;
				text = String(dateOf(day).getUTCDate());
			} else if (tick === "month" && dateOf(day).getUTCDate() === 1) {
				show = true;
				text = dateOf(day).toLocaleDateString(undefined, { month: "short", timeZone: "UTC" });
			}
			if (!show) continue;
	...
	private renderGrid(canvas: HTMLElement, minDay: number, maxDay: number, tick: "day" | "week" | "month"): void {
	...
	private renderRow(body: HTMLElement, item: TimelineItem): void {
		...
		this.registerDomEvent(bar, "click", (e) => {
			...
			void this.app.workspace.getLeaf(Keymap.isModEvent(e)).openFile(item.file);
		});
	...
	private renderUnscheduled(groups: RenderGroup[]): void {
		...
		for (const item of items) {
			...
			this.registerDomEvent(chip, "click", (e) => {
				void this.app.workspace.getLeaf(Keymap.isModEvent(e)).openFile(item.file);
			});
		}
	}
